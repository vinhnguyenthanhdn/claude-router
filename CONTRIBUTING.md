...
Removing a public function from `scripts/` is allowed, but the pull request description has to name it. `scripts/Common.ps1` and `scripts/common.sh` are dot-sourced by the launchers, by the installers and by the suites, so a name that leaves one of them breaks callers that are nowhere in the diff. `tests/scope-guard.sh` compares the functions defined at the base commit with those at your head commit and fails on any removal the description does not mention; naming it — with the reason it is going away — is enough to pass. Which files it watches is decided the same way the parse step decides, so a library added later is covered without an edit here, and the guard proves on every run that it can still fire, in both languages.
